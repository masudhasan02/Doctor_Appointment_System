    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (sessss != null && sessss.getAttribute("tempname") != null) {
                // User is logged in
                x = (int)sessss.getAttribute("otp");
                xx = String.valueOf(x);
                if (xx.equals(userOtp)) {
                    response.sendRedirect("http://localhost:8080/DoctorAppointment/serverpage/newPass.jsp");
                } else {
                    out.println("<script>alert('Wrong OTP');</script>");
                }
            } else {
                response.sendRedirect("http://localhost:8080/DoctorAppointment/serverpage/forgetPass.jsp");
            }           
