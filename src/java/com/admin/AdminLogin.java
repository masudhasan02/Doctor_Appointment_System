                    ...
                    try (OraclePreparedStatement ost = (OraclePreparedStatement) oconn.prepareStatement(q)) {
                        ...
                        try (OracleResultSet ors = (OracleResultSet) ost.executeQuery()) {
                            if (ors.next()) {
                                ...
                                sesss.setAttribute("adminname", uname);
                                // Redirect to a webpage
                                // Get the absolute path of the JSP file
                                response.sendRedirect("http://localhost:8080/DoctorAppointment/serverpage/adminHomePage.jsp");
                            } else {
                                response.sendRedirect("http://localhost:8080/DoctorAppointment/serverpage/adminLogin.jsp");
                            }
                        }
                    }
                }
